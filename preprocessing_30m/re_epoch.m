    ...
    if exist(fullfile(subject_output_data_dir, '03_ica_data', ica_rej_fname),'file')==2
        ...
        if length(find(strcmp({EEG.event.type},task_event_markers{1}))) || length(find(strcmp({EEG.event.type},task_event_markers{2})))
            ...
            if all_bad_epochs==0
                ...
                for ch=1:EEG.nbchan
                    EEG = pop_eegthresh(EEG,1, ch, volt_threshold(1), volt_threshold(2), task_epoch_length(1), task_epoch_length(2),0,0);
                    EEG = eeg_checkset(EEG);
                    EEG = eeg_rejsuperpose(EEG, 1, 1, 1, 1, 1, 1, 1, 1);
                    bad_trials= EEG.reject.rejglobal;
                    badChans(ch,:) = bad_trials;
...
%     asr_tot_samples_modified', asr_change_in_RMS', ica_prparation_bad_channels',...
