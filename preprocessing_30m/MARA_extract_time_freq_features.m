...
%     values_m(1,ic) = Hz20_50;
    ...
    [pks, locs, widths, proms] = peak_finder(resids_pxx(4:16),[4:16]); %looks for peaks btw 5 and 15 Hz (1 less and more so any bumps at 5 or 15 are caught)
...
%         if (proms(numPeaks) > 0.5 && pks(numPeaks)>1 && widths(numPeaks) > 0.9) %liberal
    ...
    [pks, locs, widths, proms] = peak_finder(pxx(5:17),[5:17]); %looks for peaks btw 5 and 15 Hz (1 less and more so any bumps at 5 or 15 are caught)
    for numPeaks = 1:length(pks) %in case we have mult. peaks
...
%         if (proms(numPeaks) > 0.35) %liberal
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if ic < 8
        [pks, locs, widths, proms] = peak_finder(resids_pxx(4:16),[4:16]);
        for numPeaks = 1:length(pks) %in case we have mult. peaks
...
%             if (proms(numPeaks) > 0.15 && pks(numPeaks)>1) %liberal
        ...
        % the code above just seemed too cautious
        % We are testing it now and might make changes
        [pks, locs, widths, proms] = peak_finder(pxx(5:17)+(10*change(5:17)),[5:17]);
        for numPeaks = 1:length(pks) %in case we have mult. peaks
