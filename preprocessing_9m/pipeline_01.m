...
for s_idx=1:length(subs)
    ...
    task_epoch_length = [-1 1.5; -.5 2.75];
    ...
    if EEG.xmax>10
        ...
        unepochedEEG = eeg_checkset(unepochedEEG);

        for e_idx=1:length(task_event_markers)
            event=task_event_markers{e_idx};
            if length(find(strcmp({unepochedEEG.event.type},event)))
                EEG = pop_epoch( unepochedEEG, {event}, task_epoch_length(e_idx,:), 'epochinfo', 'yes');
                total_epochs_before_artifact_rejection(s_idx,e_idx)=EEG.trials;
                ...
                if all_bad_epochs==0
                    total_epochs_after_artifact_rejection(s_idx,e_idx)=EEG.trials;
                    
                    %% Interpolation
                    EEG = pop_interp(EEG, origEEG.chanlocs, interp_type);
                    fprintf('\nMissed channels are spherically interpolated\n');
                    total_channels_interpolated(s_idx,e_idx)=length(origEEG.chanlocs)-length(EEG.chanlocs);

                    %% Re-referencing
                    EEG = pop_reref( EEG, []);

                    fig=compute_and_plot_psd(EEG, 1:EEG.nbchan);
                    saveas(fig, fullfile(subject_output_data_dir,sprintf('10-art_rej_reref_psd_%s.png',event)));


                    %% Save processed data
                    EEG = eeg_checkset(EEG);
                    EEG = pop_editset(EEG, 'setname',  strrep(data_file_name, ext, sprintf('_rereferenced_data_%s',event)));
                    EEG = pop_saveset(EEG, 'filename', strrep(data_file_name, ext, sprintf('_rereferenced_data_%s.set',event)),...
                        'filepath', [subject_output_data_dir filesep '04_rereferenced_data']); % save .set format
                else
                    total_epochs_after_artifact_rejection(s_idx,e_idx)=0;
                    total_channels_interpolated(s_idx,1)=NaN;
                    total_channels_interpolated(s_idx,2)=NaN;
                end
        ...
        total_epochs_before_artifact_rejection(s_idx,1)=NaN;
        total_epochs_before_artifact_rejection(s_idx,2)=NaN;
        total_epochs_after_artifact_rejection(s_idx,1)=NaN;
        total_epochs_after_artifact_rejection(s_idx,2)=NaN;
        total_channels_interpolated(s_idx,1)=NaN;
        total_channels_interpolated(s_idx,2)=NaN;
    end
    close all;
...
report_table=table(subj_ids',...
    ...
    length_ica_data', total_ICs', total_ICs_removed', total_epochs_before_artifact_rejection(:,1),...
...
report_table.Properties.VariableNames={'subject',...
    'lof_n_flat_channels', 'lof_n_channels','lof_n_periodo_channels', 'lof_n_bad_channels'...
    'asr_tot_samples_modified', 'asr_change_in_RMS','ica_preparation_n_bad_channels','ica_preparation_n_bad_epochs'...
    'length_ica_data', 'total_ICs', 'total_ICs_removed','total_epochs_before_artifact_rejection_ima1',...
    'total_epochs_before_artifact_rejection_mov1', 'total_epochs_after_artifact_rejection_ima1',...
    'total_epochs_after_artifact_rejection_mov1', 'total_channels_interpolated_ima1',...
    'total_channels_interpolated_mov1'};
